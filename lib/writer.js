...
async function callGroq(groqKey, model, messages, maxTokens = 2000) {
  const res = await fetch(GROQ_URL, {
    ...
    signal: AbortSignal.timeout(30000), // Groq is fast — 30s is plenty
  });
  ...
  if (!content) throw new Error('Empty Groq response: ' + JSON.stringify(data).slice(0, 200));
  return content;
}

// ─── GEMINI CALL (fallback) ───────────────────────────────────────────────────

async function callGemini(geminiKey, messages, maxTokens = 2000) {
  ...
  let data;
  try { data = JSON.parse(text); } catch { throw new Error('Gemini non-JSON: ' + text.slice(0, 100)); }
...
// ─── MAIN FALLBACK CHAIN: Groq → Gemini ──────────────────────────────────────

async function callWithFallback(keys, messages, maxTokens = 2000) {
  ...
  if (groqKey) {
    for (const model of GROQ_MODELS) {
      try {
        console.log(`[Writer] Trying Groq: ${model}`);
        const result = await callGroq(groqKey, model, messages, maxTokens);
        console.log(`[Writer] ✓ Groq success: ${model}`);
        return { result, model: `groq/${model}` };
      } catch (e) {
        console.warn(`[Writer] Groq ${model} failed: ${e.message.slice(0, 120)}`);
        errs.push(`groq/${model}: ${e.message.slice(0, 80)}`);
      }
    }
  }
  ...
  if (geminiKey) {
    try {
      console.log(`[Writer] Trying Gemini: ${GEMINI_MODEL}`);
      const result = await callGemini(geminiKey, messages, maxTokens);
      console.log(`[Writer] ✓ Gemini success`);
      return { result, model: `gemini/${GEMINI_MODEL}` };
    } catch (e) {
      console.warn(`[Writer] Gemini failed: ${e.message.slice(0, 120)}`);
      errs.push(`gemini/${GEMINI_MODEL}: ${e.message.slice(0, 80)}`);
    }
  }
...
async function generateOutline(apiKey, plan) {
  ...
  const { result, model } = await callWithFallback(keys, messages, 1200);
...
export async function writeArticle(keys, plan, imageSet) {
