...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('[Bot] Settings loaded — wpUrl:', settings.wpUrl, '| groq:', !!settings.groqKey, '| gemini:', !!settings.geminiKey);
  } catch (e) {
  ...
  for (let i = 0; i < plans.length; i++) {
    ...
    try {
      ...
      const written = await writeArticle({ groqKey: settings.groqKey, geminiKey: settings.geminiKey }, plan, imageSet);
